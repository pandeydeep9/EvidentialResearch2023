...
def ensure_path(path, remove=True):
    ...
    if os.path.exists(path):
        print("Removing previous path: ", path)
        time.sleep(10)
        shutil.rmtree(path)
        os.makedirs(path)
    else:
...
def compute_logits(feat, proto, metric='dot', temp=1.0):
    assert feat.dim() == proto.dim() #Overall shape --> 4,5,64 and 4, 75, 64 --> Both have 3 dimensions

    if feat.dim() == 2: #No Batches, just one task at a time
        if metric == 'dot':
            logits = torch.mm(feat, proto.t()) # Transpose proto, matrix multiply
        elif metric == 'cos':
            logits = torch.mm(F.normalize(feat, dim=-1),
                              F.normalize(proto, dim=-1).t())
        elif metric == 'sqr': #TODO: UNCLEAR FOR NOW

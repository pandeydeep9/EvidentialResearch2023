...
def get_svhn_dataset(mean, std,batch_size=16, num_workers=2, shuffle=True):
...
def compute_mean_std(svhn_dataset):
    ...
    data_r = numpy.dstack([svhn_dataset[i][1][:, :, 0] for i in range(len(svhn_dataset))])
    data_g = numpy.dstack([svhn_dataset[i][1][:, :, 1] for i in range(len(svhn_dataset))])
    data_b = numpy.dstack([svhn_dataset[i][1][:, :, 2] for i in range(len(svhn_dataset))])

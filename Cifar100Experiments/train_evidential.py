...
def one_hot_embedding(labels, num_classes=10):
    ...
    y = torch.eye(num_classes).to(args.device)
    return y[labels]
...
if __name__ == '__main__':
    ...
    for epoch in range(1, settings.EPOCH + 1):
        ...
        # if best_accr < acc:
        #     weights_path = os.path.join(checkpoint_path, f'{args.net}-best.pth')
        #     print('saving weights file to {}'.format(weights_path))
        #     torch.save(net.state_dict(), weights_path)
        #     os.mkdir(os.path.join(checkpoint_path, f"{epoch}-best.txt")
        #     # print("best: ", best_accr, acc)
        #     best_accr = acc
        #     continue

        # if not epoch % settings.SAVE_EPOCH:
        #     weights_path = os.path.join(checkpoint_path, f'{args.net}-regular.pth')
        #     os.mkdir(os.path.join(checkpoint_path, f"{epoch}-regular.txt")
        #     print('saving weights file to {}'.format(weights_path))
        #     torch.save(net.state_dict(), weights_path)
